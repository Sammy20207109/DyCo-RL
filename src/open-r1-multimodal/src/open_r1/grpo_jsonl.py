...
from open_r1.qwen2_5vl_monkey_patch import monkey_patch_qwen2_5vl_flash_attn, monkey_patch_qwen2_5vl_forward, monkey_patch_torch_load,monkey_patch_qwen2_5vl_model_forward,monkey_patch_qwen2_5vl_prepare_input,monkey_patch_qwen2_5vl_flash_attn_forward,monkey_patch_qwen2_5vl_decoder_forward
from open_r1.dyco_rl_monkey_patch import apply_dyco_rl_patches
monkey_patch_qwen2_5vl_flash_attn()
monkey_patch_torch_load()
apply_dyco_rl_patches()
...
SYSTEM_PROMPT = (
    ...
    "You are capable of interleaved image-text thinking. "
    "In the <think></think> tag, you can use <|box_start|>[x1,y1,x2,y2]<|box_end|> "
    "to focus on that region of the image. You can inspect it once or multiple times "
    "and think in an interleaved manner to answer the question. "
    "You need to answer a single-choice question. You only need to reply with "
...
#                 ##
#                 item['steps'] = item.get('steps', None)
...
#                 ]
#             }
#     def debug_make(example):
#         out = make_conversation_from_jsonl(example)
...
#     # Map the conversations (load_from_cache_file=False to ensure latest code is used)
#     dataset.map(debug_make, num_proc=1, load_from_cache_file=False)
#     #dataset = dataset.map(make_conversation_from_jsonl, num_proc=1, load_from_cache_file=False)
...
if __name__ == "__main__":
    ...
    if training_args.deepspeed and "zero3" in training_args.deepspeed:
        ...
        monkey_patch_qwen2_5vl_prepare_input()
        monkey_patch_qwen2_5vl_flash_attn_forward()
        monkey_patch_qwen2_5vl_forward()
        monkey_patch_qwen2_5vl_model_forward()
        monkey_patch_qwen2_5vl_decoder_forward()
