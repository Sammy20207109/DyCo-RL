    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            result,img_ratio,overall_topk = self._sample(
                input_ids,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                generation_config=generation_config,
        ...
        return result,img_ratio,overall_topk#,all_step_ratios
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            # total_img_ratio +=outputs.img_ratio
            # count+=1
            img_ratio=outputs.img_ratio
            all_topk_indices.append(outputs.topk_indices.unsqueeze(1))

            # # 在每次 decode 后执行
            # all_step_ratios.append({k: v.clone() for k, v in outputs.step_ratios.items()})

            del outputs
        
        #count ave img ratio
        #img_ratio=total_img_ratio/count
        overall_topk = torch.cat(all_topk_indices, dim=1)  # [batch, decode_steps, k]

        print(overall_topk.shape)
        ...
        if return_dict_in_generate:
            ...
            return input_ids,img_ratio,overall_topk#all_step_ratios
