...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        sample_score = None  # DyCo-RL: only set for SAMPLE/GREEDY mode
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            # 12. run sample (it degenerates to greedy search when `generation_config.do_sample=False`),all_step_ratios
            result, sample_score = self._sample(
                input_ids,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                generation_config=generation_config,
    ...
    # === [AT-RL v39] Lightweight attention distance helpers (M + FR) ===
    @staticmethod
    @torch.no_grad()
    def _batched_M_FR(a, b):
        """
        Compute L2 distance (M) and Fisher-Rao distance (FR) between adjacent attention vectors.
        a, b: [B, N] attention vectors
        Returns: M [B], FR [B]
        """
        a_f = a.float().clamp(min=1e-12)
        b_f = b.float().clamp(min=1e-12)
        # M: L2 distance
        M_dist = (b_f - a_f).norm(dim=1)  # [B]
        # FR: Fisher-Rao distance = 2*arccos(sum(sqrt(p*q)))
        p = a_f / a_f.sum(dim=1, keepdim=True).clamp(min=1e-12)
        q = b_f / b_f.sum(dim=1, keepdim=True).clamp(min=1e-12)
        bc = (p * q).sqrt().sum(dim=1).clamp(0.0, 1.0)
        FR_dist = 2.0 * torch.arccos(bc)
        FR_dist = torch.where(torch.isfinite(FR_dist), FR_dist, torch.zeros_like(FR_dist))
        return M_dist, FR_dist
    # === [AT-RL v39] helpers end ===
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        """DyCo-RL _sample: multinomial sampling with FR-based scoring."""
        # init values
        pad_token_id = generation_config._pad_token_tensor
        output_attentions = generation_config.output_attentions
        output_hidden_states = generation_config.output_hidden_states
        ...
        while self._has_unfinished_sequences(
            ...
            # [DyCo-RL] Collect attention signals per decode step
            _img_ratio = getattr(outputs, 'img_ratio', None)
            _text_ratio = getattr(outputs, 'text_ratio', None)
            if _img_ratio is not None:
                img_ratio_list.append(_img_ratio)
                text_ratio_list.append(_text_ratio)

            # Adjacent FR: FR(prev_attn, cur_attn)
            cur_img_attn = getattr(outputs, 'img_attn', None)
            cur_text_attn = getattr(outputs, 'text_attn', None)
            if prev_img_attn is not None and cur_img_attn is not None:
                L_txt = min(prev_text_attn.shape[1], cur_text_attn.shape[1])
                _, fr_i = self._batched_M_FR(prev_img_attn, cur_img_attn)
                _, fr_t = self._batched_M_FR(prev_text_attn[:, :L_txt], cur_text_attn[:, :L_txt])
                FR_img_list.append(fr_i)
                FR_txt_list.append(fr_t)
            prev_img_attn = cur_img_attn.detach() if cur_img_attn is not None else None
            prev_text_attn = cur_text_attn.detach() if cur_text_attn is not None else None

            del outputs
        # ======== end decode loop ========

        # Guard: no attention signals (e.g. InternVL) → zero score
        if len(img_ratio_list) == 0:
            sample_score = torch.zeros(input_ids.shape[0], device=input_ids.device, dtype=torch.float32)
            return input_ids, sample_score

        if streamer is not None:
            streamer.end()

        if return_dict_in_generate:
            zero_score = torch.zeros(input_ids.shape[0], device=input_ids.device)
            if self.config.is_encoder_decoder:
                return GenerateEncoderDecoderOutput(
            ...
            # === [DyCo-RL] Eq 6-9: FR classification + alignment score ===
            sample_score = torch.zeros(input_ids.shape[0], device=input_ids.device, dtype=torch.float32)
            if len(FR_img_list) > 0:
                FR_img_t = torch.stack(FR_img_list, dim=1)  # [B, T-1]  v^vis_t
                FR_txt_t = torch.stack(FR_txt_list, dim=1)  # [B, T-1]  v^txt_t

                # Align img_ratio/text_ratio with FR (FR skips first token)
                ir_stack = torch.stack(img_ratio_list, dim=1).float()  # [B, T]  r^vis_t
                tr_stack = torch.stack(text_ratio_list, dim=1).float() # [B, T]  r^txt_t
                T_dist = FR_img_t.shape[1]
                ir_al = ir_stack[:, 1:T_dist+1] if ir_stack.shape[1] > T_dist else ir_stack[:, :T_dist]
                tr_al = tr_stack[:, 1:T_dist+1] if tr_stack.shape[1] > T_dist else tr_stack[:, :T_dist]
                if ir_al.shape[1] < T_dist:
                    pad = T_dist - ir_al.shape[1]
                    ir_al = torch.nn.functional.pad(ir_al, (0, pad), value=ir_al[:, -1:].mean())
                    tr_al = torch.nn.functional.pad(tr_al, (0, pad), value=tr_al[:, -1:].mean())

                # Eq 6: relative normalized ratio r̃^m_t = r^m_t / (r^vis_t + r^txt_t)
                ratio_sum = (ir_al + tr_al).clamp(min=1e-8)
                ir_rel = ir_al / ratio_sum  # [B, T]  r̃^vis_t
                tr_rel = tr_al / ratio_sum  # [B, T]  r̃^txt_t

                
                FR_img_n = FR_img_t  
                FR_txt_n = FR_txt_t  

                # Eq 8: token role classification (asymmetric τ)
                # vis>txt: no threshold (actively looking at image)
                # txt>vis: need margin τ to filter noise
                tau = 0.05
                fr_diff = FR_img_n - FR_txt_n  # normalized v^vis_t - v^txt_t
                D_vis = (fr_diff > 0)      # perception: any img dominance
                D_txt = (fr_diff < -tau)    # reasoning: txt must exceed by τ
                # tokens where 0 >= fr_diff >= -tau are neutral

                # Print classification stats for first sample in batch
                n_vis = D_vis[0].sum().item()
                n_txt = D_txt[0].sum().item()
                n_total = D_vis.shape[1]
                n_neutral = n_total - n_vis - n_txt
                print(f"[DyCo-RL] Token classification (sample 0, T={n_total}): "
                      f"D_vis={n_vis}({100*n_vis/n_total:.1f}%) "
                      f"D_txt={n_txt}({100*n_txt/n_total:.1f}%) "
                      f"neutral={n_neutral}({100*n_neutral/n_total:.1f}%) "
                      f"fr_diff: mean={fr_diff[0].mean().item():.4f} std={fr_diff[0].std().item():.4f}")

                # Eq 9: alignment score s_t
                s_t = ir_rel * D_vis.float() + tr_rel * D_txt.float()  # [B, T]
                # neutral tokens get s_t = 0

                # Per-sample mean score (sigmoid to keep bounded)
                B = s_t.shape[0]
                for bi in range(B):
                    active = D_vis[bi] | D_txt[bi]
                    if active.sum() > 0:
                        sample_score[bi] = torch.sigmoid(s_t[bi][active].mean())
                    else:
                        sample_score[bi] = 0.5

                # zscore → tanh → [-1, 1]
                if sample_score.std() > 1e-8:
                    sample_score = torch.tanh((sample_score - sample_score.mean()) / sample_score.std())
                else:
                    sample_score = torch.zeros_like(sample_score)
